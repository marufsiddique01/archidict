...
// Get related terms
$relatedTerms = $termObj->getRelatedTerms($termId);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        $image = $term['image']; // Keep existing image by default
        if (isset($_FILES['image']) && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
            ...
            if ($imageValidation['success']) {
                ...
                if (move_uploaded_file($_FILES['image']['tmp_name'], $targetPath)) {
                    ...
                    if (!empty($term['image'])) {
                        $oldImagePath = TERMS_UPLOADS . $term['image'];
        ...
        if (empty($message)) {
            ...
            $result = $termObj->update($termId, $termData);
            
            if ($result === true) {
                // Term updated successfully, now set related terms
                $termObj->setRelatedTerms($termId, $relatedTerms);
    ...
    // Pre-populate form with existing term data
    $name = $term['name'];
    $slug = $term['slug'];
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h1>Edit Term: <?php echo htmlspecialchars($term['name']); ?></h1>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <?php if (!empty($term['image'])): ?>
                            <div class="mb-2">
                                <img src="<?php echo UPLOADS_URL . 'terms/' . htmlspecialchars($term['image']); ?>" alt="<?php echo htmlspecialchars($term['name']); ?>" class="img-fluid image-preview">
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="term-relationship-list">
                            <?php if (count($allTerms) > 0): ?>
                                <?php foreach ($allTerms as $relTerm): ?>
                                    <?php if ($relTerm['id'] != $termId): // Skip the current term ?>
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="related_terms[]" value="<?php echo $relTerm['id']; ?>" id="term-<?php echo $relTerm['id']; ?>" <?php echo in_array($relTerm['id'], $relatedTerms) ? 'checked' : ''; ?>>
                                            <label class="form-check-label" for="term-<?php echo $relTerm['id']; ?>">
                                                <?php echo htmlspecialchars($relTerm['name']); ?>
                                            </label>
                                        </div>
                                    <?php endif; ?>
                                <?php endforeach; ?>
